...
class Character {
    ...
    draw(index) {
        ...
        this.ctx.strokeRect((60 + this.posXX), this.posYY + 180, (cnv_i.width - 15) * this.zoom, (cnv_i.height - 55) * this.zoom);
        this.ctx.stroke();
        this.posXX += this.sprites[index].to_goX;
        this.ctx.drawImage(cnv_i, this.posXX, this.posYY, cnv_i.width * this.zoom, cnv_i.height * this.zoom);
        this.sprites[index].to_draw = 0;

    }

    drawPlayer() {
        //Ce qu'il faudra faire en gros c'est que les deux sprites ait 
        for (let i = 0; i < this.sprites.length; i += 1) {
...
let player_2 = new Character(600, 0, ctx);
    ...
    if (this.status == 200) {
        ...
        spritesheet.onload = function () {
            ...
            for (let i = 0; i < 6; i++) {
                ...
                new_sp_2[i] = new SpriteAtlas(context1, json_infos);
            }
            new_sp[0].add_anime("normal", 1, 10, "");
            new_sp[1].add_anime("punch", 1, 5, "Punch");
            new_sp[2].add_anime("walk-left", 1, 11, "WalkLeft");
            new_sp[3].add_anime("walk-right", 1, 11, "WalkRight");
            new_sp[4].add_anime("jump", 1, 11, "Jump");
            new_sp[5].add_anime("down", 1, 6, "Down");

            new_sp_2[0].add_anime("normal", 1, 10, "");
            new_sp_2[1].add_anime("punch", 1, 5, "Punch");
            new_sp_2[2].add_anime("walk-left", 1, 11, "WalkLeft");
            new_sp_2[3].add_anime("walk-right", 1, 11, "WalkRight");
            new_sp_2[4].add_anime("jump", 1, 11, "Jump");
            new_sp_2[5].add_anime("down", 1, 6, "Down");
            ...
            for (let i = 0; i < new_sp_2.length; i++) {
                player_2.sprites.push(new_sp_2[i]);
            }
            ...
            player_1.sprites[4].to_goY = -20;

            player_2.sprites[2].to_goX = +10;
            player_2.sprites[3].to_goX = -10;
            player_2.sprites[4].to_goY = -20;
